...
contract_address()
{
    ...
    local path
    path=$(dirname "$contract_file")
    local result
    result=$(tonos-cli genaddr --setkey "$signer" "$path"/"$contract_file".tvc "$path"/"$contract_file".abi.json | grep_deploy_addr)
    echo "$result"
}
...
get_contract_balance()
{
    ...
    local result
    result=$(everdev c i -n "$network" -a "$addr" | pcregrep -o1 "\((\d+) nano\)")
    echo "$result"
}
...
get_signer_pubkey()
{
    ...
    local key
    key=$(everdev s l | pcregrep -o1 "^$signer.+([a-z0-9]{64})")
    echo "$key"
}
...
decode_contract_code()
{
    ...
    tonos-cli decode stateinit --tvc "$tvc_file" | pcregrep -o1 "code\": \"(.+?)\""
}
...
decode_contract_data()
{
    ...
    assert_not_empty "$tvc_file" "tvc file missing: $tvc_file"
    tonos-cli decode stateinit --tvc "$tvc_file" | pcregrep -o1 "data\": \"(.+?)\""
}
